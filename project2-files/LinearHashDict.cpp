...
// End of "DO NOT CHANGE" Block
  ...
  // No need to delete the data, as all copied into new table.
    ...
    //oldTable points to the old table
    int oldSize = size;
    //record the old table size
    size_index++;
    //size = primes[size_index];
    size = notprimes[size_index];
    //bigger table
    table = new bucket[size]();
    //table is new table with bigger size
    number = 0;
    //number of states reset to 0
...
bool LinearHashDict::find(MazeState *key, MazeState *&pred) {
  ...
  // TODO:  Your code goes here...
    ...
    while (i < size) {
        ...
        if (table[h].key->getUniqId() == keyID) {
            record_stats(probes);
            pred = table[h].data;
            return true;
        }
