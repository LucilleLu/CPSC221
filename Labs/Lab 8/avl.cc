...
void balance( Node *& x ) {
    ...
    if ( balance < -1) {  //right is heavy
        int rightChildBalance = height(x->right->left) - height(x->right->right);
        if ( rightChildBalance > 0) {   // right left case
            doubleRotateLeft(x);
    ...
    if ( balance > 1) {  //left is heavy
        int leftChildBalance = height(x->left->left) - height(x->left->right);
        if (leftChildBalance < 0) {     // left right case
            doubleRotateRight(x);
        } else {
            rotateRight(x);             // left left case
        }
    }
}
