...
#pragma omp task untied shared(root, lword, lcount)
    findMaxHelper(root->left, lword, lcount);
    findMaxHelper(root->right, rword, rcount);
#pragma omp taskwait
    if (rcount > lcount && rcount > root->value) {
        word = rword;
        count = rcount;
    } else if (lcount > root->value) {
        word = lword;
        count = lcount;
    } else {
        word = root -> key;
