...
/*
 * Find the largest of the node
 */
Node*& find_largest(Node *& node){
    if (node -> right == NULL)
        return node;
...
bool delete_node(Node*& root, KType key) {
	...
	// case 2: target has two children
        ...
        Node* largest = find_largest(target -> left);
        Node* parent_of_largest = find_parent(target, largest);
        
        target->key = largest->key;
        
        if (parent_of_largest->left == largest){
            parent_of_largest->left = largest->left;
        } else {
            parent_of_largest->right = largest->left;
        }
        delete largest;
